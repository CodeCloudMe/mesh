 ...
 */
 ...
 module.exports = function(dirOrScript, callback) {

 	var cachedDeps = {},
 	loaded = { filesByKey: {}, files: [], modules: [] };
		...
		step(
			...
			on.success(function(content) {
				...
				function(dep, next) {
					...
					if(!depInfo.pkgPath) return next(null, [[]]);
	...
	var findRequiredDeps = exports.findRequiredDeps = function(entry, callback, root) {
		...
		step(
			...
			on.success(function(deps) {
				...
				async.map(usable, function(dep, next) {
					findRequiredDeps(dep.path, next, true);
				}, this)
			}),
			on.success(function() {
	...
	var includeRequiredThirdPartyDeps = exports.includeRequiredThirdPartyDeps = function(files, callback) {
		...
		step(
			...
			on.success(function(modules) {
				var next = this;
				async.map(modules, includeProject, outcome.error(next).success(function(result) {
					next(null, result)
				}));
			}),
	...
	var init = function(dirOrScript, callback) {
